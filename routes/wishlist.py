...
def add_to_wishlist(token: str, coin_id: str):
    ...
    user_id = user["id"]
    existing = db.table("wishlist").select(
...
def get_wishlist(token: str):
    ...
    if not user:
        return {"message": "Invalid token"}
    user_id = user["id"]
    wishlist = db.table("wishlist").select(
        "*").eq("user_id", user_id).execute().data
    if wishlist:
        coins = []
        for item in wishlist:
            coin = db.table("coins").select("*").eq("coin_id",
                                                    item["coin_id"]).single().execute().data
            coins.append(coin)
...
def remove_from_wishlist(token: str, coin_id: str):
    ...
    if not user:
        return {"message": "Invalid token"}
    user_id = user["id"]
    exists = db.table("wishlist").select("*").eq("user_id",
                                                 user_id).eq("coin_id", coin_id).execute().data
    if not exists:
        return {"message": "Coin doesn't exist in wishlist"}
    db.table("wishlist").delete().eq(
        "user_id", user_id).eq("coin_id", coin_id).execute()
