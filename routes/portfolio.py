...
def get_portfolio(token: str):
    user = db.table("users").select("id").eq(
        "login_token", token).single().execute().data
    if not user:
        return {"message": "Invalid token"}

    user_id = user["id"]
    portfolio = db.table("portfolio").select(
        "*").eq("user_id", user_id).execute().data

    for p in portfolio:
        coin = db.table("coins").select("*").eq("coin_id",
                                                p["coin_id"]).single().execute().data
        ...
        db.table('portfolio').update({"current_price": current_price, 'profit_loss': profit_loss}).eq(
            'portfolio_id', p['portfolio_id']).execute()
...
def buy_coin(token: str, quantity: int, coin_id: str):
    user_getting = db.table("users").select(
        '*').eq('login_token', token).execute()
    ...
    existing = db.table("portfolio").select(
        "*").eq("user_id", user_id).eq("coin_id", coin_id).execute().data
...
def sell_coin(token: str, quantity: int, coin_id: str):
    ...
    if remaining_qty == 0:
        db.table("portfolio").delete().eq(
            "portfolio_id", old["portfolio_id"]).execute()
    else:
        db.table("portfolio").update({
